...
# ========== СОСТОЯНИЯ ДЛЯ ПОИСКА МИШКИ ==========
class FindBearGame(StatesGroup):
    waiting_for_box = State()

user_bear_game = {}  # {user_id: {"attempts": int, "message_id": int, "start_time": float, "bear_found": bool}}
...
def get_user(user_id: int) -> Dict:
    if user_id not in users_db:
        users_db[user_id] = {
            ...
            "bear_game_completed": False
        }
        save_data()
    else:
        user = users_db[user_id]
        if "bear_game_completed" not in user:
             user["bear_game_completed"] = False
...
async def send_welcome_chain_after_game(user_id: int):
    """Отправляет приветственные сообщения 2, 3, 4 после прохождения игры"""
    user = get_user(user_id)
    
    # Отправляем сообщения со 2 по 4 (индексы 1, 2, 3)
    for i in range(1, len(WELCOME_MESSAGES)):
        msg_data = WELCOME_MESSAGES[i]
        await asyncio.sleep(msg_data["delay_seconds"])
...
def bear_boxes_keyboard(clicked_boxes: dict = None):
    """Создаёт клавиатуру с 9 коробками (3x3)"""
    if clicked_boxes is None:
        clicked_boxes = {}
    
    buttons = [
        [
            InlineKeyboardButton(text=clicked_boxes.get("1", "📦"), callback_data="bear_box_1"),
            InlineKeyboardButton(text=clicked_boxes.get("2", "📦"), callback_data="bear_box_2"),
            InlineKeyboardButton(text=clicked_boxes.get("3", "📦"), callback_data="bear_box_3")
        ],
        [
            InlineKeyboardButton(text=clicked_boxes.get("4", "📦"), callback_data="bear_box_4"),
            InlineKeyboardButton(text=clicked_boxes.get("5", "📦"), callback_data="bear_box_5"),
            InlineKeyboardButton(text=clicked_boxes.get("6", "📦"), callback_data="bear_box_6")
        ],
        [
            InlineKeyboardButton(text=clicked_boxes.get("7", "📦"), callback_data="bear_box_7"),
            InlineKeyboardButton(text=clicked_boxes.get("8", "📦"), callback_data="bear_box_8"),
            InlineKeyboardButton(text=clicked_boxes.get("9", "📦"), callback_data="bear_box_9")
        ],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
async def show_bear_game(user_id: int, chat_id: int, edit: bool = False, message_id: int = None):
    """Показывает интерфейс игры 'Найди мишку'"""
    game_data = user_bear_game.get(user_id, {"attempts": 0, "start_time": None, "clicked_boxes": {}})
    
    if game_data.get("start_time") is None:
        game_data["start_time"] = datetime.now().timestamp()
        game_data["clicked_boxes"] = {}
        game_data["max_attempts"] = 3
        user_bear_game[user_id] = game_data
    
    attempts = game_data["attempts"]
    max_attempts = game_data.get("max_attempts", 3)
    remaining_attempts = max_attempts - attempts
    
    text = (
        f"<b>Угадай коробку и получи 🧸 Мишку!</b>\n\n"
        f"🕹 Попыток: {remaining_attempts}\n"
    )
    
    keyboard = bear_boxes_keyboard(game_data.get("clicked_boxes", {}))
    
    # ССЫЛКА НА ФОТО
    photo_url = "https://i.postimg.cc/CxsYdPXD/IMG-20260504-022548-805.jpg"
...
async def show_sponsors_after_bear(user_id: int, chat_id: int):
    """Показывает спонсоров после нахождения мишки"""
    if not sponsors_db:
        return True
    ...
    text = (
        f"🧸 <b>Ты нашел Мишку!</b>\n\n"
        f"👇 <b>Теперь подпишись на каналы, чтобы получить награду:</b>\n\n\n"
    )
    
    # Создаём клавиатуру со спонсорами
    buttons = []
    for sponsor in not_subscribed:
        ...
        if link.startswith("@") or "t.me/" in link:
            if link.startswith("@"):
                ...
                url = link if link.startswith("https://") else f"https://{link}"
            buttons.append([InlineKeyboardButton(text=f"🌸 Подписаться", url=url)])
        else:
            buttons.append([InlineKeyboardButton(text=f"🌸 Подписаться", url=link)])
    
    buttons.append([InlineKeyboardButton(text="✅️ Я подписался", callback_data="check_sponsors_after_bear")])
...
# ========== ИГРА "НАЙДИ МИШКУ" ==========
@dp.callback_query(F.data == "bear_box_1")
@dp.callback_query(F.data == "bear_box_2")
@dp.callback_query(F.data == "bear_box_3")
@dp.callback_query(F.data == "bear_box_4")
@dp.callback_query(F.data == "bear_box_5")
@dp.callback_query(F.data == "bear_box_6")
async def open_bear_box(callback: CallbackQuery):
    """Открытие коробки"""
    user_id = callback.from_user.id
    box_num = callback.data.split("_")[2]
    
    if user_id not in user_bear_game:
        await callback.answer("Игра не найдена, начните заново", show_alert=True)
        return
    
    game_data = user_bear_game[user_id]
    attempts = game_data["attempts"]
    
    # Проверяем, не нашли ли уже мишку
    if game_data.get("bear_found", False):
        await callback.answer("Вы уже нашли мишку! Подпишитесь на каналы.", show_alert=True)
        return
    
    # Проверяем, не открывали ли уже эту коробку
    clicked_boxes = game_data.get("clicked_boxes", {})
    if box_num in clicked_boxes:
        await callback.answer("Вы уже открывали эту коробку!", show_alert=True)
        return
    
    # Увеличиваем счётчик попыток
    new_attempts = attempts + 1
    
    # Сохраняем, что коробка открыта (ставим крестик)
    clicked_boxes[box_num] = "❌"
    game_data["clicked_boxes"] = clicked_boxes
    game_data["attempts"] = new_attempts
    
    # Третья попытка - ВСЕГДА УДАЧНАЯ
    if new_attempts == 3:
        game_data["bear_found"] = True
        user_bear_game[user_id] = game_data
        ...
        await show_sponsors_after_bear(user_id, callback.message.chat.id)
    
    # Первая или вторая попытка (неудачные)
    user_bear_game[user_id] = game_data
    
    # Обновляем сообщение с игрой (показываем крестик на нажатой коробке)
    msg_id = game_data.get("message_id")
    if msg_id:
        await show_bear_game(user_id, callback.message.chat.id, edit=True, message_id=msg_id)
@dp.callback_query(F.data == "check_sponsors_after_bear")
async def check_sponsors_after_bear(callback: CallbackQuery):
    """Проверка подписок после нахождения мишки"""
    user_id = callback.from_user.id
    user = get_user(user_id)
    ...
    if all_subscribed or not sponsors_db:
        # Отмечаем, что пользователь прошёл игру
        user["bear_game_completed"] = True
        save_user(user_id, user)
        
        # Очищаем данные игры
        if user_id in user_bear_game:
            del user_bear_game[user_id]
        ...
        # Запускаем ВСЕ приветственные сообщения (1, 2, 3, 4)
        asyncio.create_task(send_welcome_chain(user_id, force=True))
        ...
        buttons.append([InlineKeyboardButton(text="✅ Я подписался", callback_data="check_sponsors_after_bear")])
...
async def is_user_subscribed_to_all(user_id: int) -> bool:
...
async def cmd_start(message: types.Message):
    ...
    # ЕСЛИ ПОЛЬЗОВАТЕЛЬ - АДМИН, СРАЗУ ПОКАЗЫВАЕМ ГЛАВНОЕ МЕНЮ
    if user_id == ADMIN_ID:
        await send_with_photo(
            message.chat.id,
            f"<b>{PREMIUM_FLOWER} Главное меню {PREMIUM_FLOWER}</b>",
    ...
    # Проверяем, подписан ли пользователь на всех спонсоров
    is_subscribed = await is_user_subscribed_to_all(user_id)
    
    # Если пользователь НЕ подписан на всех спонсоров ИЛИ не проходил игру
    if not is_subscribed or not user.get("bear_game_completed", False):
        # Если НЕ подписан, сбрасываем флаг прохождения игры
        if not is_subscribed and user.get("bear_game_completed", False):
            user["bear_game_completed"] = False
            save_user(user_id, user)
        
        # Отправляем ТОЛЬКО первое приветственное сообщение
        first_msg = WELCOME_MESSAGES[0]
        keyboard = create_custom_url_keyboard(first_msg["buttons"])
        await bot.send_message(
            user_id,
            first_msg["text"],
            parse_mode="HTML",
        ...
        # Сбрасываем данные игры (если были)
        if user_id in user_bear_game:
            del user_bear_game[user_id]
        
        # Запускаем игру с поиском мишки
        user_bear_game[user_id] = {
            "attempts": 0,
            "start_time": datetime.now().timestamp(),
            "bear_found": False
        }
        
        msg = await show_bear_game(user_id, message.chat.id)
        user_bear_game[user_id]["message_id"] = msg.message_id
        return
    
    # Если подписан на всех спонсоров и игра пройдена - показываем главное меню
    await send_with_photo(
        message.chat.id,
        f"<b>{PREMIUM_FLOWER} Главное меню {PREMIUM_FLOWER}</b>",
        main_menu_keyboard(user_id),
